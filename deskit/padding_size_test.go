...
func TestPaddingSizeValidation(t *testing.T) {
	...
	for _, mode := range []struct {
		...
		t.Run(mode.name, func(t *testing.T) {
			...
			plain := bytes.Repeat([]byte{0x42}, 3*des.BlockSize-1)
			ct, err := mode.encrypt(plain)
			require.NoError(t, err)
			for _, padding := range []int{-des.BlockSize, 0, 1, des.BlockSize - 1, des.BlockSize * 3 / 2, 255, 256} {
				t.Run(fmt.Sprintf("invalid=%d", padding), func(t *testing.T) {
					encrypted, err := mode.encrypt(plain, padding)
					assert.EqualError(t, err, "deskit: invalid padding size")
			...
			for _, padding := range []int{des.BlockSize, 2 * des.BlockSize, 255 / des.BlockSize * des.BlockSize} {
